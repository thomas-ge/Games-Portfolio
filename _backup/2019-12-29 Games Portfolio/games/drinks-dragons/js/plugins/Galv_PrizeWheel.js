...
Galv.PRIZE.obj = function(data) {
	...
	switch (string[0]) {
		...
		case 'r':
			var id = string.slice(1);
			return {item: {iconIndex: Number(id), id: Number(id)}};
			break;
	}
};

Galv.PRIZE.start = function() {
	// Automatically stopping the ring after a random time
	enemy = $gameVariables.value(151);
	var randTimer = Math.floor((Math.random() * 3000) + 1000);
	if (enemy == 5) { setTimeout(function(){ stopMotion = 1; }, randTimer); }
...
Galv.PRIZE.give = function() {
	if (!Galv.PRIZE.result.isCustom) {
		...
		var ringResult = Galv.PRIZE.result.item.iconIndex;

		$gameVariables.setValue(79, ringResult);
...
Scene_PrizeWheel.prototype.initializeSpin = function() {
	...
	// Hier wird die Variable ausgelesen, in der steht wieviel der aktive Spieler schon getrunken hat
	var currentChar = $gameVariables.value(13);
	currentChar += 50;
	var hadDrinks = $gameVariables.value(currentChar);

	// Festlegen der Startgeschwindigkeit des Rings
	var startSpeed = 0.004;

	// Überprüfen ob der Skill "Slow the Motion" aktiviert wurde – Falls ja, soll er die Geschwindigkeit des Rings verringern
	if (skillsActivation[$gameVariables.value(13)-1].skills[11] == 2) {
		startSpeed = startSpeed - (hadDrinks * 0.0001);
		if (startSpeed < 0.001) { startSpeed = 0.001; }
	}

	// Das ist die Variable, die die Geschwindigkeit des Rings angibt
	// Werte in denen sich die Range befindet:

	// Speed			langsam		schnell
	// Original			0.003		0.005		0.003 + Math.random() * 0.001
	// Future			0.001		0.005		startSpeed - (hadDrinks * 0.0001)
	// Drinks			40			0

	this._randSlowdown = startSpeed;
	// this._randSlowdown = 0.003 + Math.random() * 0.001;
};
...
Scene_PrizeWheel.prototype.updateTimer = function() {
	// Hier wird geschaut, ob der Skill aktiviert wurde
	var stopSkill = $gameVariables.value(161);

	this._timer += 1;
	if (this._stopping) {

		// Wenn der Stop Skill erlernt wurde, hält der Ring sofort an
		if (stopSkill == 0) {
			this._speed = Math.max(this._speed - 0.001,0);
		} else if (stopSkill == 1) {
			this._speed = Math.max(this._speed - 0.1,0);
		}
...
Scene_PrizeWheel.prototype.updateInput = function() {
	if (enemy < 5 && this._inputActive && (Input.isTriggered('ok') || TouchInput.isPressed())) {
		// stop spin motion
		AudioManager.playSe(Galv.PRIZE.seConfirm);
		this._stopping = true;
		this._inputActive = false;
	} else if (stopMotion == 1 && enemy == 5 && this._inputActive && (!Input.isTriggered('ok') || TouchInput.isPressed())) {
		// stop spin motion
		AudioManager.playSe(Galv.PRIZE.seConfirm);
		this._stopping = true;
		this._inputActive = false;
